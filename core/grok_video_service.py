...
def _is_valid_video_url(url: str, *, from_video_tag: bool = False) -> bool:
    """验证视频 URL 是否有效

    Args:
        url: 待验证的 URL
        from_video_tag: 是否从 HTML <video> 标签提取，若是则放宽扩展名检查
    """
    ...
    # 检查标准视频扩展名
    has_video_ext = any(ext in lowered for ext in (".mp4", ".webm", ".mov"))
    if has_video_ext:
        return True

    # 从 <video> 标签提取的 URL 可信度高，放宽检查
    if from_video_tag:
        return True

    # 检查是否包含 generated_video 或 video 相关路径（某些 API 返回的 URL 不含扩展名）
    if "generated_video" in lowered or "/video" in lowered:
        return True

    # 检查 base64 编码的路径（某些代理会编码路径）
    # 如 /images/p_L3VzZXJzLy4uLg== 这种模式
    if "/images/p_" in url or "/videos/p_" in url:
        return True
    ...
    if "<video" in content and "src=" in content:
        ...
        for pattern in html_patterns:
            ...
            if match:
                ...
                # 从 <video> 标签提取的 URL 可信度高
                if _is_valid_video_url(url, from_video_tag=True):
                    return url
        ...
        *,
        ...
        async def _request_once() -> Any:
            ...
            if response_text.startswith("data:"):
                # SSE 流式响应，需要解析合并
                logger.debug("[GrokVideo] 检测到 SSE 流式响应，正在解析...")
