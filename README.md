...
*   [Update] **正则库扩充**：进一步丰富了触发关键词库。
    *   新增动词：`传`、`po`、`瞅`、`望`、`赏`。
    *   新增名词：`照骗`、`福利`、`美图`。
    *   新增句式：支持 `给我看看`、`让我康康` 等命令式请求。

### v1.2.3
*   [Feat] **Debug模式**：新增了 `#` 前缀调试功能。发送 `#发一张照片` 或 `#看看穿搭`，插件将拦截请求并返回最终生成的 System Prompt 内容，方便开发者调试 Prompt 结构。

### v1.2.1
*   [Fix] **摄影模式开关逻辑**：修改了 `enable_custom_camera` 的行为。
    *   **关闭 (默认)**：现在会**完全移除** Prompt 中的“摄影模式切换 (Section 6)”部分，将镜头控制权完全交给模型自由发挥。
    *   **开启**：注入包含自拍/全身/默认的完整逻辑，并使用自定义参数。

### v1.2.0
*   [Feat] **自定义镜头开关**：新增了 `enable_custom_camera` 配置项。
    *   **开启时**：允许用户自定义“自拍”和“全身”模式的 Prompt 参数。
    *   **关闭时 (默认)**：强制使用插件内置的最佳实践参数，降低配置难度。
*   [UI] **配置面板优化**：整合了拍摄模式说明到开关提示中，移除了冗余的说明文本框。

### v1.1.5
*   [Doc] **配置指引增强**：在配置面板中新增了 `cam_intro` 说明项，详细解释了“自拍/全身/默认”三种拍摄模式的自动触发机制，方便用户理解。
